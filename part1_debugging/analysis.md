...
1) Two Separate Commit Calls
   The endpoint performs two separate database commits. If the inventory commit fails, the product may still be created, leading to inconsistent system state and a product existing without inventory data.
...
7) Proper HTTP Response Code
   Returning 200 OK on resource creation is incorrect.
